...
Default.commonCSP = function (Env) {
    ...
    return [
        ...
        // Update CSP adding Parsec protocols
        // for accounts.cryptpad.fr authentication and cross-domain iframe sandbox
        Env.enableEmbedding?  `frame-ancestors 'self' ${Env.protocol} ${Env.customProtocol}`: `frame-ancestors 'self' ${domain}`,
        `worker-src 'self'`,
        ""
        // </Parsec customization>
    ];
...
Default.contentSecurity = function (Env) {
    // <Parsec customization>
    // Update CSP adding 'unsafe-inline' for script-src to allow inline scripts because of our custom frame system
